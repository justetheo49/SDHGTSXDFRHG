...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.mentions.members.first()){
        if(!args[0]){
            ...
            message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
                ...
                channel.send(new Discord.MessageEmbed() 
                    ...
                    .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec [LoveYou] !
                    Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser [LoveYou], si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.
                    **Vous êtes sur le serveur principal de LoveYou. Ici vous pouvez faire confiance au Haut gradé**`)
                    .setColor(config.embedColor)
                )
    
                channel.send(new Discord.MessageEmbed() 
                    .setTitle("Utilisation du bot de boumco")
                    .setDescription("__Voici les commandes que vous pouvez utiliser avec [LoveYou] :__")
        ...
        message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
            ...
            channel.send(new Discord.MessageEmbed() 
                .setTitle(`**Bievenue dans votre salon privé, ${target.user.username}**`)
                .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec bot [LoveYou] (By SlyClub)!
                Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser bot [LoveYou], si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.
                **Vous êtes sur le serveur principal de LoveYou. Ici vous pouvez faire confiance au Haut gradé**
                **Il se peut que vous ayez reconrtré un serveur qui copie le notre, merci de nous le signaler !**`)
                .setColor(config.embedColor)
            )

            channel.send(new Discord.MessageEmbed() 
                .setTitle("Utilisation de [LoveYou]")
                .setDescription("__Voici les commandes que vous pouvez utiliser avec [LoveYou] :__")
                .addField(config.prefix + "pub token_bot", "Cette commande sert à envoyer un message à tous les membres d'un serveur.")
                .addField(config.prefix + "puball token_bot", "Cette commande sert à envoyer un message à tous les membres des serveurs.")
                .addField(config.prefix + "pub* token_bot", "Cette commande sert à envoyer un message à tous les membres en ligne d'un serveur.")
